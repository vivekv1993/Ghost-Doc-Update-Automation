...
#Deepest Base Components (Leaves)
...
class Splitter(BaseModel):
    ...
    size: Optional[int] = Field(None, description="Size parameter for the splitter, if applicable.")

class NameValueSplitter(BaseModel):
    pattern: str = Field(description="The delimiter separating a name from a value, usually '='.")
    regex: Optional[str] = Field(None, description="Set to 'yes' if the pattern is a regular expression.")

# --- SUB-FIELDS (Recursive Container) ---
...
class NamedFieldItem(BaseModel):
    ...
    # Named fields can also hold sub-components
    sub_fields: Optional[SubFields] = None
    sub_chars: Optional[SubChars] = None
...
class LogField(BaseModel):
    # Core identifying attribute required by the blueprint rules
    id: str = Field(description="The numeric string ID matching the log-field element, e.g., '14' or '61'.")
    
    # Version boundaries and custom tracking markers from schema_map
    ...
    # Text documentation body block
    doc: Optional[str] = Field(None, description="The plain text or HTML paragraph explaining what this log field tracks.")
    
    # Parsing instruction rule block
    parse_info: Optional[ParseInfo] = None
    
    # Snap-in targets allowing fields to carry advanced sub-components
    enums: Optional[Enums] = None
    bitmask: Optional[Bitmask] = None
    sub_chars: Optional[SubChars] = None
    sub_fields: Optional[SubFields] = None
...
class LogFieldGroup(BaseModel):
    ...
    # Field groups contain an array of nested standalone log-fields
...
class LogLineVersionUpdate(BaseModel):
    ...
    # Versions contain lists of fields or field groups
    standalone_fields: Optional[List[LogField]] = Field(default_factory=list, description="Standalone fields updated within this version block context.")
    log_field_groups: Optional[List[LogFieldGroup]] = Field(default_factory=list, description="Grouped fields modified under this version boundary.")
...
class LogLineUpdate(BaseModel):
    ...
    name: Optional[str] = Field(None, description="The full descriptive name of the log line category, e.g., 'Client Requests To Ghost'.")
    ...
    # A log line can hold sub-versions, standalone fields, or field groups directly
...
class ChangelogEntry(BaseModel):
    # Strict enforcement per Option B choice: No default values to ensure clean author tracking
...
class CompilerPayload(BaseModel):
    action_type: Literal["append_changelog", "update_logline"] = Field(
        description="The target route trigger selector. Set to 'append_changelog' for history logging; set to 'update_logline' for structural configurations."
    )
    changelog_payload: Optional[ChangelogEntry] = Field(
        None, description="Populate this configuration object ONLY if action_type is evaluated exactly as 'append_changelog'."
    )
    logline_payload: Optional[LogLineUpdate] = Field(
        None, description="Populate this configuration object ONLY if action_type is evaluated exactly as 'update_logline'."
    )
